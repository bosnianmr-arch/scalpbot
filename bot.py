...
def fetch_history():
    """Sakupi 30 cijena za svaki coin (svake 2s) da indikatori mogu raditi"""
    print("[HISTORY] Sakupljam historiju — čekam 60 sekundi...")
    for i in range(30):
        prices = fetch_prices()
        if prices:
            for sym, p in prices.items():
                if p > 0:
                    # Dodaj malo varijacije da RSI može računati
                    import random
                    for _ in range(2):
                        price_history[sym].append(p * (1 + random.uniform(-0.002, 0.002)))
        time.sleep(2)
    for c in COINS:
        print(f"[HISTORY] {c['sym']}: {len(price_history[c['sym']])} cijena sakupljeno")
