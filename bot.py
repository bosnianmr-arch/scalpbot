...
def fetch_history():
    """Uzmi zadnjih 50 svjećica (1m) sa Binance za svaki coin"""
    print("[HISTORY] Učitavam historiju cijena...")
    for c in COINS:
        try:
            res = requests.get(
                'https://api.binance.com/api/v3/klines',
                params={'symbol': c['binance'], 'interval': '1m', 'limit': 50},
                timeout=10
            )
            if res.ok:
                klines = res.json()
                price_history[c['sym']] = [float(k[4]) for k in klines]
                print(f"[HISTORY] {c['sym']}: {len(price_history[c['sym']])} svjećica učitano")
        except Exception as e:
            print(f"[HISTORY] {c['sym']} failed: {e}")
